...
\documentclass[acmtoplas]{acmart}
\usepackage{booktabs}
\usepackage{listings}
\usepackage{amsmath}
...
% for TEA OS session dumps/logs
% Define a custom style for TEA OS logs
\lstdefinestyle{teaos}{
  ...
  framerule=0.4pt,
  xleftmargin=0.1\textwidth,         % center by margins
  xrightmargin=0.1\textwidth,
  linewidth=0.9\textwidth,           % 80% of page width
...
%\includepdf[pages=1,  width=\paperwidth,  height=\paperheight, pagecommand={}]{resources/pdf/frontcover.pdf}
...
\keywords{Language Engineering, Alphabet-based Languages, Language Semantics, Cross-Platform Development, Applied Transformatics}
...
\ccsdesc[100]{Applied computing~Education}
...
\begin{table}[ht]
  \centering
  \caption{What an AI Search might tell you about TEA (Transforming Executable Alphabet) right now}
  % scale table to 80% of text width
  \begin{tabular}{|p{0.8\textwidth}}

    TEA (Transforming Executable Alphabet) is based on the idea of an instruction set whose core is just the 26 letters of the Latin Alphabet, and which we can qualify or decorate using a specific set of symbols known as command qualifiers, \texttt{.}, \texttt{!}, \texttt{*} and \texttt{@}, so as to specify `qualified commands'. \\

  \end{tabular}
\end{table}

TEA began as a mobile-first text-processing prototype in 2021 and has evolved into a cross-platform language with interpreters in JavaScript (web, mobile-web) and Python (elsewhere), a growing standard library, and a documented instruction set (the TEA TAZ \cite{Lutalo2024TEATAZ}). The language targets scenarios where compactness, offline capability, and text-centric transformations are primary concerns: mobile scripting, embedded text-processing, lightweight data engineering, and domain-specific sequence analysis and transformations among others.

In \textbf{\hyperref[SEC2]{Section \ref{SEC2}}} we shall look at the goals behind how TEA was designed, then look at its actual design in \textbf{\hyperref[SEC3]{Section \ref{SEC3}}}. \textbf{\hyperref[sec:formal-semantics]{Section \ref{sec:formal-semantics}}} then drills down into the language mechanics underlying TEA including how TEA relates to the mathematics of Transformatics. In \textbf{\hyperref[SEC4]{Section \ref{SEC4}}} we then consider the actual reference implementations of the language and also present the original design of how TEA program processing would work (the basis of current TEA transpilers). \textbf{\hyperref[SEC5]{Section \ref{SEC5}}} presents some highlights of what has been accomplished with TEA thus far, with \textbf{\hyperref[SEC7]{Section \ref{SEC7}}} focusing on a few specific noteworthy case studies, and we then dive into preliminary language evaluation thus far in \textbf{\hyperref[SEC6]{Section \ref{SEC6}}}. We shall also briefly discuss TEA's observed strengths and weaknesses in \textbf{\hyperref[SEC8]{Section \ref{SEC8}}}, and shall contrast the language to earlier and on-going research in software language engineering in \textbf{\hyperref[SEC9]{Section \ref{SEC9}}}. We conclude in \textbf{\hyperref[SEC10]{Section \ref{SEC10}}} by also highlighting what future directions work on TEA is likely to take. The extra material in  \textbf{\hyperref[SEC11]{Appendix 1}} has been added to help illuminate, but also motivate interested researchers towards understanding what Transformatics is all about as well as directions to take for those interested in picking up this new mathematics by the same inventor of TEA.
...
\begin{itemize}
  ...
  \item \textbf{Conciseness.} Allow [even complex] programs to be expressed as single strings when needed (minified form) --- as \textit{one-liners}.
...
Also, note that each letter denotes a \emph{command space} (e.g., \texttt{I:} for I/O, \texttt{N:} for numeric utilities, \texttt{R:} for rewrite and math etc.) --- see details in \textbf{\hyperref[tab:tea-az-minimal]{Table \ref{tab:tea-az-minimal}}}. Qualifiers modify semantics (for example, \texttt{r.:EXPR} is for evaluating math expressions, while \texttt{r:PAT:SUB} is for text rewriting \cite{Lutalo2024TEATAZ}). However, despite the global maximum number of distinct qualified commands being bounded, and yet, in practice, not all of these are currently being utilized. Conventionally, unutilized or undefined TEA commands are labeled or considered \textbf{INERT} \cite{Lutalo2024TEATAZ}, generally have no effect in a TEA program, and might be flagged at runtime as part of dynamic program validation or debugging \cite{Lutalo2025debug}.

\section{Formal Semantics}
\label{sec:formal-semantics}

This section gives a concise, mathematically grounded semantics for TEA suitable for a SLE paper.  We combine the language definition excerpted from the TEA TAZ \cite{Lutalo2024TEATAZ} with a compact operational model based on \emph{sequence-transformer chaining} \cite{Lutalo2025_transformatics_thesis}. 
...
Reference interpreters\footnote{Actually, technically correct to be called ``transpilers" for the case of how processing TEA is currently being implemented.} exist in Python (command-line) and JavaScript (web). The interpreters implement a modular pipeline:
\begin{enumerate}
...
The TEA GitHub repository hosts the reference implementations, sample programs, and documentation. However, interested users and students might immediately access a live TEA language software operating environment (SOE) \cite{lutalo2023dnapsoe} (akin to a \textit{language workbench} or integrated development environment (IDE)) via: \url{https://tea.nuchwezi.com}


\subsection{TEA Language Processor}
...
\begin{figure*}[ht]
  ...
  %\includegraphics[trim=2cm 8cm 2cm 8cm, clip, width=0.9\textwidth,]{resources/pdfs/ProteinSynthesisStateMachine.pdf}\\
   \includegraphics[trim=0cm 1cm 0cm 0cm, clip, width=0.9\textwidth,]{resources/pdf/TEA_PROGRAM_EXEC_PROCESS.pdf}\\
...
\begin{figure}[ht]
  \begin{center}
   \includegraphics[width=0.4\textwidth]{resources/images/tea_ide.png}\\
   \caption{Latest \textbf{TEA MOBILE IDE}}
  \label{FIGTEAIDE}
  \end{center}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{40+ sample programs\footnote{Readily accessible, usable via \textbf{TEA WEB IDE} but also via the \textit{offline-capable} \textbf{TEA MOBILE IDE} \url{https://bit.ly/teamobile_apks}}:} Examples spanning calculators, data engineering, chatbots, games, cryptography, and a proof-of-concept TEA OS --- refer to \textbf{\hyperref[APPENDIX2]{Appendix 2}}.
  ...
  \item \textbf{Mobile app (mobile TTTT):} A compact TEA IDE for Android enabling offline development and running or sharing TEA programs and their outputs. See \textbf{\hyperref[FIGTEAIDE]{Figure \ref{FIGTEAIDE}}}.
\end{itemize}
...
\begin{table*}[ht]
  ...
  \begin{tabular}{@{}p{0.08\textwidth} p{0.20\textwidth} p{0.64\textwidth}@{}}
    \toprule
...
An example output from the latest ZHA (v1.0.8) for a conversation between a fictitious character ``Manuela" and the user (lines with no leading identifying handle) is shown in \textbf{\hyperref[FIGZHA]{Figure \ref{FIGZHA}}}.
...
\begin{itemize}
  ...
  \item \textbf{Type and correctness:} The dynamic, transformer-based model used in TEA requires careful testing and formal verification for safety-critical domains especially given that in TEA, there essentially is just one type of data; text or rather strings \cite{Lutalo2024TEATAZ}.
...
TEA intersects with minimal instruction-set languages, concatenative languages, and domain-specific text-processing languages \cite{lutalo2024software}. It draws inspiration from prior work on language design\cite{coblenz2018interdisciplinary}, DSL design, and the theory that many programming tasks reduce to text/sequence processing (see Lutalo's, \textit{Unifying Programming Language Theory} (UPLT) in \cite{lutalo2024software}). Additional related projects include DNAP \cite{lutalodnapintro2020,lutalo2023dnapsoe} and other lightweight scripting languages.


Though not directly related to work in Computing nor Mathematics, TEA source code has been featured as part of the narrative in the 2025 creative fiction work from Uganda --- the novel \textit{ROCK `N' DRAW} \cite{lutalo2025rock} --- as one of the potent technologies being utilized by one military command engineer for cases such as automating remote reconnaissance, drone-auto-piloting and other wartime computational loads.


TEA as a language is founded on the idea of the sequence transformer chaining paradigm, and this idea has been well formalized and developed by the same person inventing TEA, but also the new subfield of mathematics being known as `Transformatics' \cite{Lutalo2025_transformatics_thesis, lutalo_2025_trans_genetics, Lutalo2025transpsy, lutalo_2025_grimlumtauto, lutalo_2025_trans101_1page, transformatics}.
...
Barash (ACM SLE '20) \cite{barash2020exampledriven} proposes an example‑driven approach to language definition and somewhat supports a commendable model for helping newbies master otherwise tricky [meta-] languages and new technologies via the concept of programming-by-example \cite{yessenov2013colorful}. The paper argues that this lowers the barrier for beginner language engineers and outlines requirements for a web‑based tool supporting illustrative syntax definition. However, especially for his presentation of arguments on why many modern languages need great language support tools --- so-called language workbenches and IDEs being a core case he covered, it should be noted that research such as his did inspire a great deal of the work that went into designing the reliable, portable and beginer-friendly integrated development environment for the TEA language for both mobile and web users (See \textbf{\hyperref[FIGTEAIDE]{Figure \ref{FIGTEAIDE}}}). Also, his perspective complements TEA’s emphasis on concise, example‑rich documentation.
...
\begin{itemize}
  ...
  \item Performance optimizations based on standardized language benchmarks, but also via rigorous industrial testing and language adaptation case studies.
...
\section*{Appendix 1: Quick Primer on Transformatics}
\label{SEC11}

\noindent Presented here based on the original 1-pager version \cite{lutalo_2025_trans101_1page} of \cite{Lutalo2025_transformatics_thesis}, the essential foundations of the mathematics underlying TEA --- basically \textbf{transformatics}, are as such:
...
\begin{lstlisting}[style=teaos,caption={TEA OS v1.1.0 Work Session}]
...
% inner cover
% or explicitly stretch to page size
%\includepdf[pages=1,  width=\paperwidth,  height=\paperheight,  pagecommand={}]{resources/pdf/innercover.pdf}
...
% or explicitly stretch to page size
%\includepdf[pages=1,  width=\paperwidth,  height=\paperheight,  pagecommand={}]{resources/pdf/backcover.pdf}
