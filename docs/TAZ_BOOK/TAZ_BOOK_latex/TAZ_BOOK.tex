...
\item Sorting a list of labeled number expressions (in the format \texttt{LABEL1:N1, LABEL_2:N2, ..., LABEL:N,... etc.} ) in descending order of their associated numbers, but ensuring that the labels are left intact in the final sorted list.
...
Otherwise, the program as of \textbf{version 2} is as such...
...
%\begin{figure}[H]
{
  \begin{tcolorbox}[teaterminalstyle, title=TEA Program: Ranking Utility v.2, breakable]
  \begin{lstlisting}[language=TEA,breaklines=true,numbers=left]
######################################
...
i!:{ORIGINAL DATA:
   ...
   \end{lstlisting}
  \end{tcolorbox}
    \captionof{figure}{TEA APPLICATION: Ranking Utility (RU) v2}
  \label{FIGOEX2}
  \vspace{1cm}
...
So, to appreciate this tool, we can for example assume we have the following small, but somewhat representative and quirky datatset conforming to the constraints we laid out in the specification:
  ...
  \begin{tcolorbox}[title=Sample Well-formatted INPUT Dataset of 7 Labeled Numerical Expressions,  breakable,listing options={basicstyle=\ttfamily, breaklines=true, breakatwhitespace=true}]
...
And so, if we select the RU Function 2: sorting numerical labeled data in descending order, we would obtain results such as:
  ...
  \begin{tcolorbox}[title=Sample Well-formatted OUTPUT from Task 2 of RU,  breakable,listing options={basicstyle=\ttfamily, breaklines=true, breakatwhitespace=true}]
...
E:5, 88:5, A:5, A:3.2121, TEST_ty:2, LABEL_1:1, C:-5
 ...
 While, if we use Task 3: \textbf{sorting and positionifying} labeled numeric data, against the same dataset, we would obtain...
  ...
  \begin{tcolorbox}[title=Sample Well-formatted OUTPUT from Task 3 of RU,  breakable,listing options={basicstyle=\ttfamily, breaklines=true, breakatwhitespace=true}]
----------***----------
ORIGINAL DATA:
E:5,A:3.2121,LABEL_1:1,C:-5,88:5,TEST_ty:2,A:5
----------***----------
...
And as for Task 1: merely sorting a list of numeric expressions in descending order, we would have something like:
  ...
  \begin{tcolorbox}[title=Sample Well-formatted OUTPUT from Task 1 of RU,  breakable,listing options={basicstyle=\ttfamily, breaklines=true, breakatwhitespace=true}]
----------***----------
ORIGINAL DATA:
