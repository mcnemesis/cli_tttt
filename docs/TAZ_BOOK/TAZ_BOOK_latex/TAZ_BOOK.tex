...
| j:lPROMPT # and loop
   \end{lstlisting}
  \end{tcolorbox}
    \captionof{figure}{TEA APPLICATION: ZEE HACKER ASSISTANT v1.0.2}
  \label{FIGF}
  \vspace{1cm}
...
The basic application is going to be called \textbf{E-MAILER}, and the essential, fully-tested source-code for version 1.5 of our next standard TEA application is as such:

\vspace{2em}

{
 \Large
  \begin{tcolorbox}[teaterminalstyle, title=TEA Program: E-MAILER v1.5, breakable]
  \begin{lstlisting}[language=TEA,breaklines=true,numbers=left]
#/usr/bin/env tttt -fc
#######################################
v:vABOUT:{@@@@@@| E-MAILER v1.5 |@@@@@@
...
g*!:vNL:vPREAMBLE:vEMAILPreview:vMENU_ACTION | v:vPREVIEW_PROMPT
...
x!:{"reply-to": [{"email": "}| x*!:vMailFrom | x!:{"}],}
# the rest of the parameters...
x!:{"subject":"}| x*!:vMailSubject | x!:{",}
x!:{"text_content":"}| x*!:vMailBody | x!:{"} | x!:"}"
...
j:lRESET
   \end{lstlisting}
  \end{tcolorbox}
  \caption{TEA APPLICATION: E-MAILER v1.5 --- a simple and special E-Mail sending client written using the TEA language}
  \label{FIGWAPP1}
}
...
\item To work-around such restrictions, we implemented our own email sending endpoint via \texttt{https://utility.nuchwezi.com/sendemail} that somewhat accepts a payload aligned with AHASend's v2 payloads for the \textbf{conversational messaging} --- such that, instead of allowing our users to directly send as any email address they wish, we instead send on their behalf (as \texttt{noreply@nuchwezi.com}), and instead use their \textbf{TO} and \textbf{FROM} as the \textbf{to} and \textbf{cc} of our AHASend payload. Works pretty well, and is less likely to be abused or flagged as SPAM.\footnote{In retrospect, we figured that this somewhat is an experimental security fall-back to option, and that in the future, we might  decide to instead go ahead and allow to set user provided FROM and TO parameters as is. Gives the best user-experience, but might be readily abused... we shall need to study use-case further. Also, this might call for a switch of the e-mail routing provider --- meaning, we either implement our own SMTP (\textbf{Simple Mail Transfer Protocol}) processors as part of say the utility.nuchwezi.com service, or perhaps use another provider other than the European/Austrian AhaSend.com}
...
\begin{enumerate}
\item It prevents the usual TEA GUI from being presented when the program detects we are running TEA on the command-line. Essentially, all calls to \texttt{i*:} are merely swapped with calls to \texttt{i:}
...
\vspace{2em}
  ...
  \begin{tcolorbox}[teaterminalstyle, title=TEA Application: Free Dictionary | LEXICON v2, breakable]
  \begin{lstlisting}[language=TEA,breaklines=true,numbers=left]
#!/usr/bin/env tttt -fc
v:vABOUT:{
*****************************
...
*****************************
Use this app to simply look-up terms or build a list of definitions.
It is based on Dictionary API that mostly focuses on standard terms, 
thus, some queries might have no resolutions or there might be service outages.
Sometimes try-again if a definition failed to load. Feel free to proceed...}

# Windows/Microsoft could be under-attack (noticed while testing this app)
# thus this [hack-proof] version
# otherwise, original minimal version of TEA LEXICON v1 is at:
...
---[ INTRO ]
# generally, use no gui-for CLI environments 
# (noticed some issues with underlying Py/TK for CLI-TEA)
y:vENVIRONMENT | f!:CLI:lNCLI1
y:vABOUT | i: | j:lDONE1
l:lNCLI1
y:vABOUT | i*:
l:lDONE1
...
y:vENVIRONMENT | f!:CLI:lNCLI2
y:vQUERY_PROMPT | i: | t.!: | v:vQUERY | j:lDONE2
l:lNCLI2
y:vQUERY_PROMPT | i*: | t.!: | v:vQUERY 
l:lDONE2
...
y:vENVIRONMENT | f!:CLI:lNCLI3
y:vANSWER | i: # present current definition... j:lDONE3
l:lNCLI3
y:vANSWER | i*:
l:lDONE3
...
j:lQUERY_PROMPT
   \end{lstlisting}
  \end{tcolorbox}
      \captionof{figure}{TEA Program Source-Code: Free Dictionary | LEXICON v2 --- addressing noticed environment glitches for CLI}
...
\subsection{EXAMPLE 1: \textbf{blah blah:} blah blah}
\label{SECYEXAMP1}
