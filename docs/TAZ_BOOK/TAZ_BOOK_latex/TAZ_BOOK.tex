...
\begin{enumerate}
\item \textbf{complex}: short (10-12 characters), random composition of symbols, letters and numbers. \textit{[hard to memorize, difficult to guess]}
\item \textbf{humane}: short (10-12 characters), random composition of words and symbols. \textit{[easy to memorize, theoretically hard to crack]}
\item \textbf{advanced}: long (12-20 characters), random words, numbers, symbols and [internal] white-space. \textit{[easy to memorize, hard to crack, provably secure]}
\end{enumerate}
}
\item For the case of complex mode, the user shall have the option of specifying their own symbol alphabet. (Defaults: \textbf{!@\#\$\%\^{}*()-=\_})
\item For the case of humane mode, the user shall have the option of specifying their own symbol alphabet to use to punctuate the word tokens. (Defaults: \textbf{+-=})
\item For the case of advanced mode, the user shall have the option of specifying their preferred minimum password length. (Default: 16)
\item For all the three modes, the SPG shall offer the user the option to present just the password, or the password and its security properties (length/sequence-cardinality(SC), sequence entropy (SE) and symbol set cardinality (SSC)).
\end{enumerate}
