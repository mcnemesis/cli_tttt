...
\end{tabular}
\caption{General Objectives of G:}
  \label{TABTAZG}
\end{table}
...
\subsection{EXAMPLE 1: A SERIOUS WORD COUNTER PROGRAM}
\label{SECEXAMP1G}
...
\begin{table}[H]
  ...
  \LARGE
	...
	\item Help introduce new-lines into text based on a pattern
...
\end{tabular}
\caption{General Objectives of H:}
  \label{TABTAZH}
\end{table}
...
\subsection{EXAMPLE 1: A TEXT ADVENTURE GAME: BOOBS `N' TRAPS\cite{lutalo_2025_rpg}}
\label{SECEXAMP1H}
...
\end{tabular}
\caption{General Objectives of I:}
  \label{TABTAZI}
\end{table}
...
Finally, note that I: is not only the ONLY way to block a program and prompt for input from the user, but is also the only means to make a TEA process pause, while it optionally displays some [useful] message (whatever is current AI). It makes TEA programs INTERACTIVE. A great example of I: in action is the \textbf{ZHA}\cite{lutalo2025unraveling} q-AGI personal assistant that ships with standard TEA (tttt) package on Linux/Unix.
 ...
 Concerning using the GUI-friendly \textbf{I*..} TEA commands. Note that, for all practical purposes, apart from the fact that \textbf{I*:} and \textbf{I*!:} both present an input prompt window without any prompt message (if invoked when also AI is empty or unset) --- which defers from \textbf{I:} and \textbf{I!:} behavior () --- otherwise, all \textbf{I*..} commands work the same for TEA anywhere; essentially, they render a GUI window prompting the user for some input, which is then set as the AI/IO.
...
\subsection{EXAMPLE 1: [pure-GUI] TEXT ADVENTURE GAME: BOOBS `N' TRAPS\cite{lutalo_2025_rpg}}
\label{SECEXAMP1I}
...
\begin{figure}[H]
  \begin{center}
   \includegraphics[]{resources/images/bnt_cli_original.png}\\
...
\subsection{EXAMPLE 2: The LUMTAUTO Occult Language Message Encoder}
\label{SECEXAMP12}
...
\begin{table}[H]
  ...
  \LARGE
	...
	\item Process TEA \textbf{LABEL BLOCKS}
...
\end{tabular}
\caption{General Objectives of J:}
  \label{TABTAZJ}
\end{table}
...
\subsection{EXAMPLE 1: THE MENTAL HEALTH SELF-ANALYSIS TOOL}
\label{SECEXAMP1J}
...
\begin{table}[H]
  ...
  \LARGE
	...
	\item \textbf{Process AI at line-level} (only TEA instruction that does this)
...
\end{tabular}
\caption{General Objectives of K:}
  \label{TABTAZK}
\end{table}
...
\begin{table}[H]
  ...
  \LARGE
	...
	\item \textbf{Facilitate Conditional Branching} (only TEA instruction that does this)
	\item Create Code-Blocks/Label-Blocks
	\item TEA facility to enable code encapsulation
	\item A Primitive Procedure/Function Facility
	\item Allow Something like Procedural Polymorphism
	\end{enumerate}\\
	\hline
	              
\end{tabular}
\caption{General Objectives of L:}
  \label{TABTAZL}
\end{table}
...
\subsection{EXAMPLE 1: TEXT-BASED BRAIN-GAME: LUCKY 101}
\label{SECEXAMP1L}
