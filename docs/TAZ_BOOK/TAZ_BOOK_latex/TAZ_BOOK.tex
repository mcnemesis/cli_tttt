...
\item [\textbf{Bonus}] A means for our TEA programs to interact with or operate alongside other kinds of executables in the OS environment.
\end{enumerate}
...
The link depicted in \textbf{\hyperref[FIGTEAOSPROPOSAL]{Figure \ref{FIGTEAOSPROPOSAL}}} being the first to have been officially shared with members of the TEA community concerning this proposal, we shall have it point, not to the running instance of the TOS, but rather, to the potential staging source-file for the reference implementation of this operating system. And thus, those zealously interested in following what the latest working version of the single-file TEA OS code looks like --- a program that one should be able to readily load into either the TEA WEB IDE or run on any system with \texttt{tttt} package installed and run, shall be found at the link: \url{https://bit.ly/run_teaos}
