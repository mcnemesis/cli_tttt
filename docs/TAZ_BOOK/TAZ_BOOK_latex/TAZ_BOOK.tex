...
\subsection{EXAMPLE 1: \textbf{INFORMATION SECURITY:} SECURE Password Generator (SPG)}
\label{SECEXAMP1V}


In approaching our secure password generator (SPG) system, first, we shall want to agree on a minimal specification upon which the system should be based. For starters, it shall be worth having in mind some of the state-of-the-art theory and standard recommendations regarding password design and usage best-practices. A quick consultation\cite{copilot_assistant} and some background research surface some of the following useful guidelines...
...
v:vABOUT:{
#####--[ RSPG v1.2 ]--#####
Random Secure Password Generator is a robust computer program 
meant to facilitate the generation of high quality passwords 
that are sometimes not only provably hard to crack, but which 
are also quite easy for the human to memorize. The theory and 
specifications underlying RSPG are in the TEA TAZ. 
...
TIP ABOUT Complex Mode Passwords: they are short & cryptic;
10-16; random; symbols, letters, numbers. Hard to memorize, 
difficult to guess. Great for use with password manager tools. 
ALSO: IN CASE you wish to customize symbols used 
[defaults: !@#$%^()-=_], enter your own seed set here:}
...
TIP ABOUT Humane Mode Passwords: they are long but legible; 
20-30; random; words, numbers, symbols. Easier to memorize, 
difficult to guess.  Theoretically harder to crack. ALSO: 
IN CASE you wish to customize symbols used 
[defaults: @$%+-=], enter your own seed set here:}
...
TIP ABOUT Advanced Mode Passwords: they are longer but somewhat readable; 
20+; random; cryptic words, numbers, symbols. Memorizable, hard to guess. 
Provably hard to crack. Provably Very Secure! ALSO: 
IN CASE you wish to force  a length lower limit [default: 20], 
enter your own here (between 20-50):}
...
\textbf{How to Access and RUN RSPG:}

\vspace{1em}

To quickly load, share or run this RSPG implementation (or the latest), try using any of the following links:

\vspace{2em}

\begin{enumerate}
\item Find RSPG on the list of standard TEA programs via \url{https://tea.nuchwezi.com}
\item Directly load and run RSPG via: \url{https://tea.nuchwezi.com/?i=.&fc=https://gist.githubusercontent.com/mcnemesis/19e5ec10e7a4243c098cc35bb3fa1b89/raw/rspg.tea}
\item Use the official shortened link to share or load RSPG: \url{https://bit.ly/tea_rspg}
\end{enumerate}

\vspace{2em}

Please note that, just like with all other TEA applications we have encountered, the RSPG can be used as is from virtually anywhere TEA can run, but especially because it is interactive in the presented implementation, modifications would be necessary if the underling RSPG algorithm is meant to be used as such in some non-interactive scenarios such as in the generation of secure passwords inside of other programs or scripts --- such as in server-side environments.
