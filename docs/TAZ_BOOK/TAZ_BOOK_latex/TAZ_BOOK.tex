...
Unlike the previous example application that we covered in the last section which focused on a kind of ``divinatory" statistical analysis of data (generative in nature), this particular application instead focuses on objective analysis of a given dataset using the major baseline statistical measures (statistics) and some basic machine learning techniques relating to extrapolating datasets based on analysis of existing or training data.


\vspace{2em}


For example, given the dataset as such:



  \begin{tcolorbox}[title=Sample INPUT Dataset of 50 Random Numbers (Ages),  breakable,listing options={basicstyle=\ttfamily, breaklines=true, breakatwhitespace=true}]
88, 37, 45, 86, 84, 24, 39, 97, 28, 45, 89, 10, 18, 27, 52, 66, 84, 41, 63, 17, 20, 36, 80, 85, 83, 21, 60, 72, 94, 23, 68, 14, 81, 65, 51, 25, 60, 15, 100, 50, 58, 65, 52, 53, 35, 39, 79, 15, 43, 10
  \end{tcolorbox}
  %\caption{The TEA Instruction Lexical Specification (regular language)}
  %\label{FIG1}
...
Be able to let the user select which kind of analysis they wish to have: 

descriptive: generate a graph representing the data
explolatory: compute all basic statistics: range, mode, median, mean, variance, standard deviation, modal sequence statistic.
predictive: next two numbers
