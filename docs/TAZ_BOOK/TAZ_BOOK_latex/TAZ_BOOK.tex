...
%\comment{
 ...
 & {\large \textbf{EXPERIMENTAL: CLI-TEA-specific [for now]}}\\
...
Finally, note that I: is not only the ONLY way to block a program and prompt for input from the user, but is also the only means to make a TEA process pause, while it optionally displays some [useful] message (whatever is current AI). It makes TEA programs INTERACTIVE. A great example of I: in action is the \textbf{ZHA}\cite{lutalo2025unraveling} q-AGI personal assistant that ships with standard TEA (tttt) package on Linux/Unix.
 ...
 First, a note about requirements to make these [still-experimental extensions] of the TEA language's interaction layer work:
...
For now, these features are best left for the command-line or native TEA use-cases --- as the \textbf{I*..} facility isn't yet implemented for WEB TEA. Thus, ensure to use them with clear intent about where the associate programs are meant to be used --- just like how, the \textbf{Z:/ZAP} primitive needs be used with care for functionality involving sourcing-extra power/capabilities from the environment (host system).
