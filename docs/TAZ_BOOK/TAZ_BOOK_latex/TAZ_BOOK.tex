...
\makecell[l]{s*:vNAME:vSALT:vN:vN2} & Same as s:SALT:N:N2, but operating on the string stored in the vault with the name vNAME  instead of AI, and using string stored in vSALT as the salt. All parameters are read from vaults with the specified names.\\

\rowcolor{lightgray}\bfseries \makecell[l]{s*!:vNAME:vREGEX:vN:vN2} & Same as s!:REGEX:N:N2, but operating on the string stored in the vault with the name vNAME  instead of AI.  All parameters are read from vaults with the specified names.\\
...
\vspace{2em}
   ...
   \begin{tcbverbatim}[title=UGANDA National Anthem Vowel Hymn]
O Uaa! a o uo ee,
e a ou uue i  a.
Uie, ee,
o ie
