...
\fancyhead[L]{IONA Inner Journal, Vol. 2: \textbf{GRIMOIRE LUMTAUTO}}
...
\normalsize
To manifest the arch-angel LEVIATHAN, note that the necessary sigil would be based on the SOURCE: \hl{\texttt{LEAVITHN}}\footnote{\textbf{IMPORTANT:} The careful analyst shall realize that, especially for this example, but perhaps also others, one might have more than one competing SOURCE or rather MSS with which to work; by the two-criteria original MSS method, we obtain the source ``ALEVITHN", while, by either PMSS or BMSS, we obtain this ``LEAVITHN". Consult \textbf{\hyperref[SECMSSANALYSIS]{Section \ref{SECMSSANALYSIS}}} for details!}, and so that, the necessary expression via OZIN is as:
...
\begin{alg}[The \textbf{Positional Modal Sequence Statistic} (PMSS) Algorithm]
\label{ALGPWFM}
$ $\\
\begin{enumerate}
\item \textbf{GIVEN} the input ordered sequence of symbols as $\Theta$.
\item \textbf{COMPUTE} the corresponding \textbf{unspecific symbol set}\cite{ossipaper}: $ \hat{\psi}(\Theta)$, also from which we can derive just the basic symbol set $\psi(\Theta)$.
\item \textbf{COMPUTE} a mapping of each of the unique symbols in $\psi(\Theta)$ to its \textbf{first-occurrence position} (FOP) in the unspecific symbol set --- i.e $\breve{i} : I(\theta_i \in \psi(\Theta), \hat{\psi}(\Theta)) = \breve{i}$
...
\section{Further Analysis and Comparison of MSS, BMSS and PMSS}
...
\begin{table}[htp]
  \begin{tabular}{|p{0.95\textwidth}} % Left border only
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.8\textwidth]{resources/netizens_compare_mss_methods.jpg}\\
    \end{figure} \\
    \cline{1-1} % Bottom border only
  \end{tabular}
\end{table}
...
Concerning BMSS and PMSS, note that, the PMSS algorithm (\textbf{\hyperref[ALGPWFM]{Algorithm \ref{ALGPWFM}}}) also has the extra allure that, it somewhat would never necessitate nor involve processing of fractions or fractional numbers as one might have to do when using the BMSS method. And for that reason alone, it might be more preferable for those computing MSS values by-hand or manually. But also, because of sometimes tricky quirks in platform-specific handling of floating-point numbers and such, even for computer programs computing the MSS, \textbf{in case we don't use an algorithm that can compute the MSS based on sorting using the original composite criteria}, then the PMSS method is preferable --- mostly for its algebraic and numerical simplicity. However, as these are all new innovations we have identified and developed while conducting research in \textbf{applying Transformatics to Magick}, who knows... perhaps magicians prefer having more than one way to skin a cat, but, more research, applications and further analysis shall tell, which of these methods of computing an MSS eventually wins or if all survive unto posterity!


\vspace{2em}

And then, \textbf{IMPORTANT} to note that, there is a subtle but significant difference between computing the modal sequence of some sequence using the original method using no ranking metric --- essentially, as demonstrated in \cite{transformatics}, the original paper to present this statistical measure for the first time, or as applied or demonstrated in ``Applying Transformatics in GENETICS"\cite{lutalo_2025_trans_genetics} --- a treatise on genetics, that among other things, demonstrated how to use the MSS to help visualize genetic code as genome expressions. The significant difference stems from the way the final processing step of any MSS algorithm is approached --- when we approach it as in the original formulation, we essentially proceed by sorting using a composite key: descending by $f_i$, and upon ties, ascending by $i^*$. Whereas, in the new methods proposed, applied and formalized in the current work, both BMSS and PMSS introduce a rank metric that replaces the original composite key with just a single key, and thus, we can derive the MSS via a single criteria --- the value of the computed Rank $R_i$. This later approach has its advantages as we have just explored, and discussed at length, however, it might sometimes not be what one wants. For example, we can see the difference between these three methods, when applied to a sample of words and phrases as shown in the following table, and as computed using our TEA computer programming language's \textbf{Uniqueify} command space\cite{Lutalo2024TEATAZ}:

\vspace{2em}

\begin{tabular}[t]{|l|c|c|c|}
\hline
\makecell[l]{\textbf{INPUT}\\\textbf{SEQUENCE}} & \makecell[c]{\textbf{MSS}\\\texttt{u!:}} & \makecell[c]{\textbf{BMSS \& PMSS}\\\texttt{u!.:}} & \texttt{SIMILAR?}\\
\hline
TEST & TES & TES & \texttt{TRUE}\\
ABBABCC & BAC & ABC & \texttt{FALSE}\\
LEVIATHAN & ALEVITHN & LEAVITHN& \texttt{FALSE}\\
JESUS & SJEU & JESU& \texttt{FALSE}\\
ILOVEYOU & OILVEYU & OILVEYU& \texttt{TRUE}\\
MATHEMATICS & MATHEICS & MATHEICS& \texttt{TRUE}\\
programming & rgmpoain & rgpomain & \texttt{FALSE}\\
hello world & lohe wrd & lohe wrd& \texttt{TRUE}\\
\makecell[l]{THIS is a\\TESTING TEST} & T SIEHisaNG & T SIHisEaNG& \texttt{FALSE}\\
1234567890 & 1234567890 & 1234567890 & \texttt{TRUE}\\
\hline
\end{tabular}
\caption{Comparing MSS to BMSS and PMSS for analyzing \textbf{10 caes} at character/symbol level}
\label{TABCOMPMSSMETHODS}


\vspace{2em}

So, clearly, based on the 10 cases we have analyzed in the above table, one can judge that, especially where there are duplicated symbols in a sequence, there is \textbf{likely} to be differences between computing the modal sequence using the original method as opposed to using the alternative BMSS or PMSS methods. Moreover, unless one first tests using say a manual computation as we saw in \textbf{\hyperref[FIGEXASOIMSS]{Figure \ref{FIGEXASOIMSS}}} or perhaps by approaching it as we have done in \textbf{\hyperref[TABCOMPMSSMETHODS]{Table \ref{TABCOMPMSSMETHODS}}} using TEA --- which one can readily do by going to the TEA WEB IDE (\textbf{\url{https://tea.nuchwezi.com}}) and running the MSS commands on their input/sequence to decide how best to proceed --- one might not know if the difference shall matter.
...
\textbf{NOTE: Computing Modal Sequence Statistics using TEA}\\

%draw horizontal line
\noindent\rule{\textwidth}{0.4pt}

For those interested in the kind of analysis we have covered in this chapter, but also who wish to apply the ideas we have presented in this grimoire concerning use of the statistic measure of the modal sequence statistic, note that the TEA (Transforming Executable Alphabet) programming language now offers the following different methods of computing the MSS or PMSS at symbol/character level, but also at word level as shown below:

\vspace{2em}

\textbf{Computing MSS using Original Method:}

\begin{enumerate}
\item \texttt{u:},\texttt{u:STR}, \texttt{u*:vNAME}  --- compute the MSS for a sequence of words.
\item \texttt{u!:},\texttt{u!:STR}, \texttt{u*!:vNAME}  --- compute the MSS for a sequence of characters.
\end{enumerate}

\vspace{2em}

\textbf{Computing MSS using PMSS Method:}

\begin{enumerate}
\item \texttt{u.:},\texttt{u.:STR}, \texttt{u*.:vNAME}  --- compute the PMSS for a sequence of words.
\item \texttt{u!.:},\texttt{u!.:STR}, \texttt{u*!.:vNAME}  --- compute the PMSS for a sequence of characters.
\end{enumerate}

\vspace{2em}

Details about what the various command signatures indicated above mean --- such as why it is that \texttt{u*!.:vNAME} works or is recognized, yet \texttt{u!*.:vNAME} wont work, or how to go about applying them in various contexts, etc. are in the official work-book/programmer's manual for the language; the \textbf{TEA TAZ} --- refer to \cite{Lutalo2024TEATAZ} and/or via the language's GitHub project repository\cite{cli_tttt}.
