...
BY THE TIME THE SUN SETS TODAY, I WANT TO HAVE AN EXTRA 100 DOLLARS ON MY ACCOUNT
...
\begin{multline}
\Theta_{soi} = \langle \text{BY THE TIME THE SUN SETS TODAY,} \\\text{I WANT TO HAVE AN EXTRA 100 DOLLARS ON MY ACCOUNT} \rangle
\end{multline}
...
\textbf{STEP 2: Reduce the SOI to the ``SOURCE" still spanning} $\psi_{36}$. In our illustration here, we are reducing using the method of computing a Modal Sequence Statistic\cite{Lutalo2025_transformatics_thesis} as shown in the computations happening in the table depicted in \textbf{\hyperref[FIGEXASOISIGILA]{Figure \ref{FIGEXASOISIGILA}}}, and which we also reproduce here as \textbf{\hyperref[TABEXASOISIGILA]{Table \ref{TABEXASOISIGILA}}}
...
The value `` TAENOSYHIMUDR0LCB,WVX1" being what we obtain, when we use the TEA programming language program code --- just \texttt{u!:} --- to compute the corresponding MSS value for our example SOI; also refer to program source-code in \textbf{\hyperref[FIGTEAEXASOISRC]{Figure \ref{FIGTEAEXASOISRC}}}.
 ...
 %\small
  ...
  %\begin{lstlisting}[language=TEA, caption={TP C7}, label={LSTC7}, numbers=left]
  \begin{lstlisting}[language=TEA,breaklines=true]
i!:{BY THE TIME THE SUN SETS TODAY, I WANT TO HAVE AN EXTRA 100 DOLLARS ON MY ACCOUNT}
u!:

#RESULTANT MEMORY STATE: (= TAENOSYHIMUDR0LCB,WVX1, VAULTS:{})
   \end{lstlisting}
  \end{tcolorbox}
...
#RESULTANT MEMORY STATE: (=TAENOSYHIMUDR0LCBWVX1, VAULTS:{})
   \end{lstlisting}
  \end{tcolorbox}
...
And so, without reproducing that table here manually --- the method used, being well documented and illustrated in \textbf{Section 4.1} of \cite{transformatics}, and also summarized in \textbf{Proposal 5} of \cite{Lutalo2025_transformatics_thesis} --- let us also realize that, if, instead of reducing to a modal sequence statistic, we had chosen to reduce to just the set of unique letters, with their order sorted alphabetically, then, we would essentially have the SOURCE as ``ABCDEHILMNORSTUVWXY" --- a \textbf{Lexical Basis} (after reducing some numbers to letters, removing punctuation, etc). as shown using just the TEA program instruction \texttt{b!:} as shown in 
...
#RESULTANT MEMORY STATE: (=ABCDEHILMNORSTUVWXY, VAULTS:{})
   \end{lstlisting}
  \end{tcolorbox}
